...
import { 
  ...
  ShieldAlert, Trash2, Check, Crown, Info, Zap, X, GitMerge, ScanLine, ChevronDown, RefreshCw, Copy, Layers, Package
} from 'lucide-react';
...
const normalizeValue = (val: any, smartNum: boolean, strict: boolean): string => {
...
const DuplicatesTab: React.FC<Props> = ({ fileData, addLog, onReset, language = 'en' }) => {
          ...
          if (tData.length > 0) {
              ...
              headers.forEach((h, sIdx) => {
                  if (selectedCols.includes(sIdx)) {
                      const tIdx = tHeads.findIndex(th => 
                        th && h && String(th).toLowerCase().trim() === String(h).toLowerCase().trim()
                      );
  ...
  const analyzeRowsLogic = (inputRows: ProcessedRow[]) => {
      ...
      if (mode === 'self') {
          ...
          newRows.forEach(row => {
              let key = "";
              if (checkFullRow) {
                 key = row.data.map(c => normalizeValue(c, smartNumberMatch, strictMatch)).join("|||");
              } else {
                 key = selectedCols.map(c => normalizeValue(row.data[c], smartNumberMatch, strictMatch)).join("|||");
              }
          ...
          targetData.forEach(tRow => {
              ...
              selectedCols.forEach(sIdx => {
                  ...
                  if (tIdx !== undefined && tIdx !== -1) {
                      const val = normalizeValue(tRow[tIdx], smartNumberMatch, strictMatch);
                      keyParts.push(val);
          ...
          newRows.forEach(row => {
              ...
              let isValidKey = false;
              selectedCols.forEach(sIdx => {
                  const val = normalizeValue(row.data[sIdx], smartNumberMatch, strictMatch);
                  keyParts.push(val);
                  if (val) isValidKey = true;
              });
  ...
  const handleAutoResolve = () => {
      ...
      setRows(result.rows);
      setStats(result.stats);
      addLog(`Resolved ${resolvedCount} entries.`, 'success');
  };
  ...
  const handleExport = async () => {
      ...
      // Prepare All Data
      const allRowsData = rows.map(r => [...r.data, r._status, r._clusterId || ""]);

      // BATCH EXPORT LOGIC
      if (exportBatchSize > 0 && allRowsData.length > exportBatchSize) {
          setStatus(ProcessingStatus.PROCESSING);
          addLog(`Splitting into files of ${exportBatchSize} rows...`, 'info');
          ...
          try {
              ...
              for (let i = 0; i < total; i += exportBatchSize) {
                  const chunkData = allRowsData.slice(i, i + exportBatchSize);
                  const exportData = [exportHeader, ...chunkData];
                  
                  const wb = XLSX_STYLE.utils.book_new();
                  const ws = XLSX_STYLE.utils.aoa_to_sheet(exportData);
                  // Basic Styles for chunk
                  ...
                  zip.file(`Checked_Part_${part}.xlsx`, buffer);
                  part++;
              }
              ...
              link.download = `Checked_Batch_${fileData.name}.zip`;
              link.click();
      ...
      // SINGLE FILE EXPORT (Existing)
      const exportData = [exportHeader, ...allRowsData];
      const wb = XLSX_STYLE.utils.book_new();
      const ws = XLSX_STYLE.utils.aoa_to_sheet(exportData);
      const range = XLSX_STYLE.utils.decode_range(ws['!ref'] || "A1");
  ...
  return (
    <div className="space-y-4 h-[calc(100vh-140px)] flex flex-col relative">
      ...
      <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex flex-col gap-3 shrink-0 z-20">
          ...
          <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-3 ml-auto">
                  <label className="flex items-center gap-1 cursor-pointer text-xs font-medium text-slate-600" title="Check strict equality (Case Sensitive)">
                      <input type="checkbox" checked={strictMatch} onChange={e => setStrictMatch(e.target.checked)} className="rounded text-red-600"/>
                      Strict Match
                  </label>
                  <label className="flex items-center gap-1 cursor-pointer text-xs font-medium text-slate-600" title="Treat 1.0 same as 1">
      ...
      {/* 2. MAIN CONTENT (Keep existing layout) */}
