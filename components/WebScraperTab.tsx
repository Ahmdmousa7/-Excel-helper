...
const WebScraperTab: React.FC<Props> = ({ addLog, onReset, language = 'en' }) => {
  ...
  const fetchContent = async (targetUrl: string): Promise<{ text: string; source: 'html' | 'markdown' | 'json' }> => {
     try {
        ...
        try {
           ...
           console.warn("Direct Jina fetch failed, trying proxy...", directErr);
        }
     ...
     const proxies = [
        ...
        },
        {
           name: "CorsAnywhere",
           fetch: async (u: string) => {
              const res = await fetch(`https://cors-anywhere.herokuapp.com/${u}`);
              if (!res.ok) throw new Error(`Status ${res.status}`);
              return await res.text();
           }
        }
  ...
  const handleScrape = async () => {
    ...
    const customList = customFields.split(',').map(s => s.trim()).filter(s => s);
    const combinedFields = [...activeFields, ...customList];

    let queryInstruction = instruction;
    ...
    try {
      ...
      if (source === 'markdown') {
          ...
          if (nextData) {
             ...
             if (finalCleanText.length > 500000) finalCleanText = finalCleanText.slice(0, 500000);
          } else {
      ...
      const result = await aiService.extractStructuredData(finalCleanText, queryInstruction);
        ...
        <div className="lg:col-span-1 space-y-4">
           <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
              ...
              <div className="space-y-4">
                 ...
                 <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase mb-2 flex items-center gap-2"><CheckSquare size={14}/> Fields to extract</label>
                    <div className="flex flex-wrap gap-2 mb-3">
                      {Object.keys(fields).map((f) => (
                        <label key={f} className="flex items-center space-x-2 text-xs bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-md cursor-pointer hover:bg-slate-100 transition-colors select-none">
                           <input 
                             type="checkbox" 
                             checked={fields[f]} 
                             onChange={() => setFields(prev => ({...prev, [f]: !prev[f]}))} 
                    ...
                    <input 
                      type="text" 
                      placeholder="Custom fields e.g. SKU, Brand, Stock Status (comma separated)"
                      value={customFields}
                      onChange={(e) => setCustomFields(e.target.value)}
                      className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-500 outline-none mb-4"
                    />
                 </div>
                 ...
                 <div>
                    ...
                    <textarea 
                      placeholder={language === 'ar' ? "مثال: استخرج قائمة المنتجات مع التفاصيل." : "Additional instructions (optional)"}
                      value={instruction}
