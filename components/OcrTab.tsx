...
import { extractFromMedia, extractStructuredData } from '../services/geminiService';
import { TRANSLATIONS, Language } from '../utils/translations';
import ProgressBar from './ProgressBar';
import { 
  ScanText, Download, UploadCloud, FileText, Trash2, 
  Zap, Edit3, Table as TableIcon, X, Plus,
  TableProperties, Split, DollarSign, Type, FileSpreadsheet, ArrowUpDown, Search, Eye, Layers, Box, Boxes, ClipboardPaste
} from 'lucide-react';
...
const generateRandomSku = () => `AM-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
...
const OcrTab: React.FC<Props> = ({ addLog, onReset, language = 'en' }) => {
  ...
  const ensureSku = (row: any) => {
      // List of keys to check for an existing identifier
      const idKeys = ['SKU', 'sku', 'Barcode', 'barcode', 'ID', 'id', 'Item Code', 'ItemCode', 'item_code'];
      
      // Find existing non-empty value
      let existingVal = "";
      for (const k of idKeys) {
          if (row[k] && String(row[k]).trim()) {
              existingVal = String(row[k]).trim();
              break;
          }
      }

      // If we didn't find one, generate random
      if (!existingVal) {
          existingVal = generateRandomSku();
      }

      return {
          ...row,
          SKU: existingVal // Enforce 'SKU' key
      };
  };
  ...
  const handleExtract = async () => {
    ...
    if (inputMode === 'paste' && !pastedText.trim()) {
        addLog("Please paste text to extract.", 'warning');
        return;
    }
    ...
    try {
        if (inputMode === 'paste') {
            ...
            const result = await extractStructuredData(pastedText, prompt);
            if (result && result.length > 0) {
                const enriched = result.map((row, i) => ensureSku({
                    ...row,
                    _SourceFile: 'Pasted Text',
                    _FileId: `text-${i}`
                }));
                allResults.push(...enriched);
                addLog("Text extraction successful.", 'success');
            } else {
                throw new Error("No structured data found in text.");
            }
            setProgress(100);
        } else {
            ...
            for (let i = 0; i < updatedFiles.length; i++) {
                ...
                try {
                    ...
                    if (result && result.length > 0) {
                        const enriched = result.map(row => ensureSku({
                        ...row,
        ...
        addLog(`Extraction Failed: ${e.message}`, 'error');
        setStatus(ProcessingStatus.ERROR);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
         ...
         <div className="lg:col-span-4 space-y-4">
            ...
            <button
               ...
               disabled={(inputMode === 'upload' && files.length === 0) || (inputMode === 'paste' && !pastedText) || status === ProcessingStatus.PROCESSING}
               className={`w-full py-3 rounded-lg font-bold text-white shadow-sm flex justify-center items-center space-x-2
                  ${(inputMode === 'upload' && files.length === 0) || (inputMode === 'paste' && !pastedText) || status === ProcessingStatus.PROCESSING
         ...
         <div className="lg:col-span-8 flex flex-col min-h-[500px]">
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg w-fit mb-4">
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-xs font-bold transition-all ${inputMode === 'upload' ? 'bg-white text-blue-600 shadow' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    <UploadCloud size={14}/> Upload Files
                </button>
                <button
                    onClick={() => setInputMode('paste')}
                    className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-xs font-bold transition-all ${inputMode === 'paste' ? 'bg-white text-blue-600 shadow' : 'text-slate-500 hover:text-slate-700'}`}
                >
            ...
            {masterData.length === 0 ? (
               inputMode === 'upload' ? (
                   ...
                   <div className="flex-1 bg-white p-6 rounded-lg border-2 border-dashed border-slate-300 hover:border-blue-400 hover:bg-slate-50 transition-all text-center flex flex-col items-center justify-center">
                       <textarea 
                           className="w-full h-full p-4 bg-transparent border-none outline-none resize-none text-sm font-mono placeholder-slate-400"
                           placeholder="Paste your raw text, invoice content, or menu text here..."
                           value={pastedText}
               ...
               <div className="flex flex-col h-full bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="flex-1 flex overflow-hidden">
                     {/* Data Table */}
                     <div className={`flex-1 overflow-auto ${showSplitView && inputMode === 'upload' ? 'w-1/2 border-r border-slate-200' : 'w-full'}`}>
                     ...
                     {/* Image Viewer (Split Mode) - Only available in upload mode */}
                     {showSplitView && inputMode === 'upload' && (
