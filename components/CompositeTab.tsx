      ...
      for (let i = 0; i < totalRows; i += chunkSize) {
         ...
         chunk.forEach((row, cIdx) => {
            ...
            if (activeTab === 'validate') {
                ...
                // NEW RULE: Check Max Ingredients (Limit 10)
                // Count pairs in dynamic section
                let pairCount = 0;
                for (let k = fixedColCount; k < row.length; k += 2) {
                    const iSku = String(row[k] || "").trim();
                    const iQty = String(row[k+1] || "").trim();
                    // If either SKU or Qty is present, we consider it a "pair" usage
                    if (iSku || iQty) {
                        pairCount++;
                    }
                }
                ...
                if (pairCount > 10) {
                    rowErrors.push(`Too many ingredients (${pairCount}). Max allowed is 10.`);
                    // Highlight the start of the 11th pair (Column Index = fixed + 20)
                    rowLocations.push(getCellRef(fixedColCount + 20)); 
                }
                ...
                // Numeric Check - Only for Quantity columns (Odd indices in dynamic section)
                rowVals.forEach(item => {
                   if (item.col >= fixedColCount) {
                      // Dynamic structure: [SKU] [Qty] [SKU] [Qty] ...
                      // Relative Index:     0     1     2     3 ...
                      // Even = SKU (Allow String), Odd = Qty (Must be Number)
