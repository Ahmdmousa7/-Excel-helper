...
describe('the digest is deterministic', () => {
  it('does not depend on the order files are supplied in', () => {
    const forward = buildBody(base);
    const reversed = buildBody({ ...base, files: [...base.files].reverse() });
    expect(reversed).toBe(forward);
    expect(digestBody(reversed)).toBe(digestBody(forward));
  });
  ...
  it('does not depend on object key order', () => {
    ...
    expect(buildBody(shuffled)).toBe(buildBody(base));
  });

  it('produces the same digest across repeated runs with no timestamp drift', () => {
    // A timestamp in the body would break this, which is why there isn't one:
    // the digest has to be a function of the reviewed content alone.
    expect(digestBody(buildBody(base))).toBe(digestBody(buildBody(base)));
  });

  it('emits LF only, so a CRLF checkout cannot change the digest', () => {
    expect(buildBody(base)).not.toContain('\r');
  });

  it('sorts paths by UTF-8 bytes, not UTF-16 code units', () => {
    // U+1F600 is a surrogate pair in UTF-16 and sorts before U+FF5E there,
    // but after it in UTF-8. The manifest must use the byte order.
    ...
    const body = buildBody({ ...base, files: [{ path: emoji, oid: oid('3') }, { path: wide, oid: oid('4') }] });
    const rows = body.trimEnd().split('\n').slice(-2);
    expect(rows[0]).toContain(wide);
    expect(rows[1]).toContain(emoji);
  });
});
...
describe('the digest detects tampering', () => {
  ...
  it('rejects an appended file row', () => {
    const text = renderAttestation(base) + `${oid('7')} sneaky.ts\n`;
    expect(() => parseAttestation(text)).toThrow(/file count mismatch/);
  });
});
...
describe('round-tripping', () => {
  ...
  it('refuses an unknown format version', () => {
    const text = renderAttestation(base).replace(MAGIC, 'apexyard-review-attestation v99');
    expect(() => parseAttestation(text)).toThrow(/unsupported attestation format/);
  });

  it('refuses a body with no separator', () => {
    expect(() => parseAttestation('just some text\n')).toThrow(/no "--" separator/);
  });
});

describe('input validation', () => {
  it('rejects a path containing a newline, which could forge a row', () => {
  ...
  it('rejects a malformed oid', () => {
    expect(() => buildBody({ ...base, files: [{ path: 'a.ts', oid: 'nope' }] }))
      .toThrow(/invalid oid/);
  });

  it('accepts a 64-char oid, for a SHA-256 repository', () => {
    expect(() => buildBody({ ...base, files: [{ path: 'a.ts', oid: 'a'.repeat(64) }] }))
      .not.toThrow();
  });

  it('rejects an unknown gate level', () => {
    expect(() => buildBody({ ...base, gate: 'whenever' })).toThrow(/unknown gate level/);
  });

  it('rejects a negative finding count', () => {
    expect(() => buildBody({ ...base, findings: { high: -1 } })).toThrow(/non-negative/);
  });

  it('defaults absent severities to zero rather than omitting them', () => {
    const body = buildBody({ ...base, findings: {} });
    expect(body).toContain('findings critical=0 high=0 medium=0 low=0 info=0');
  });
});
