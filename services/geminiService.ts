...
const isQuotaError = (error: any): boolean => {
  ...
  // 1. Check Top-Level Code/Status
  const code = error.status || error.code || error.response?.status;
  if (Number(code) === 429) return true;
  if (Number(code) === 503) return true; 

  // 2. Check Nested Error Object
  if (error.error) {
...
const parseWaitTime = (error: any): number => {
  const msg = (error.message || error.error?.message || JSON.stringify(error)).toLowerCase();
  const match = msg.match(/retry in (\d+(\.\d+)?)s/);
  if (match && match[1]) {
    return Math.ceil(parseFloat(match[1])) + 1; // Add 1 second buffer
  }
  return 60; // Default to 60s if parsing fails
};
...
export const verifyGeminiKey = async (keyString: string): Promise<'valid' | 'invalid' | 'quota'> => {
  ...
  try {
    ...
    // Use Flash for verification as it's cheaper/faster
...
export const translateBatch = async (
  ...
  // Infinite attempts allowed for Quota waits, but hard limit for other errors
  let attempts = 0;
  ...
  while (true) {
    try {
      ...
      // --- FAILOVER: GROQ (Optional, if configured) ---
      if (isQuota && groqApiKey && attempts > 1) {
      ...
      if (isQuota) {
        ...
        // No keys left, MUST WAIT
        const waitSeconds = parseWaitTime(error);
        console.warn(`Quota limit reached on all keys. Waiting ${waitSeconds} seconds...`);
        // Wait the full duration + 1s buffer
        await new Promise(r => setTimeout(r, waitSeconds * 1000));
        continue; // Retry same request
      }
      
      // If not quota error, or too many non-quota retries
  ...
  while (true) {
    try {
      ...
      if (isQuota) {
        ...
        const waitSeconds = parseWaitTime(error);
        console.warn(`Quota limit reached. Waiting ${waitSeconds} seconds...`);
        await new Promise(r => setTimeout(r, waitSeconds * 1000));
        continue;
      }
      if (attempts > 3) throw error;
