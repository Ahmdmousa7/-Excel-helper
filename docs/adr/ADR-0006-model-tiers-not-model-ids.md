...
- **A fallback is no longer silent.** `translateBatch` takes an `onNotice` callback; `TranslateTab` puts the message in the log and at the top of the exported workbook's Translation Summary sheet. A `console.warn` was the only record before, which is to say none — the file looked identical to one produced entirely on Pro. Verified in a browser against an endpoint that returns a retirement 404 for `gemini-3.1-pro` — an id which, it turned out two days later, never existed. The test was still valid (a nonexistent id and a retired one produce the same NOT_FOUND, which is exactly why `isModelUnavailable` cannot distinguish them), but it is worth recording that the mock was made of the same fiction as the list: a fallback test cannot tell you whether the thing you are falling back *from* is real. Only `scripts/list-gemini-models.mjs` can.
