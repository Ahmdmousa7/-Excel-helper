...
| `firestore.rules` deployed status unverified (TD-026) | Still unverified — the *project* outlives the client; TD-026 stays open |
...
- **No credential of any kind is left in the client** — but the key is not gone from the world. `firebase-applet-config.json` is deleted along with the Firebase web API key it carried, which was never a secret (it is a project identifier). It remains readable in git history and in every bundle served before this deploy, so removing it from `HEAD` is tidiness, not revocation. Revoking it means acting in the console: TD-026.
- **TD-026 is NOT closed, and this ADR does not close it.** It tracked whether `firestore.rules` was actually deployed and whether the API key had referrer restrictions. Deleting the client does **not** delete the Firebase project, does not change the deployed ruleset, and does not invalidate the API key — which stays in git history and in every bundle already served. The exposure moved from the app to the Google console. TD-026 is reworded to say so, and its fix is now "delete the project", which is finally possible because nothing depends on it. **Deferred on 2026-08-13** at the maintainer's request, with revisit triggers recorded in the register — still open, deliberately not yet done.
